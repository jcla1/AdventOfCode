...
const mapBoard      = f => R.map(R.map(f)),
      cleanBoard    = mapBoard(R.max(0)),
      access        = (board, [i, j]) => board[i][j],
      boardIndicies = board => {
        const height = R.length(board),
              width  = R.length(board[0]);
        return R.chain(i => R.map(j => [i, j], R.range(0, width)), R.range(0, height));
      };
...
const flash = board => {
  const flashPoints = R.filter(p => access(board, p) >= 10, boardIndicies(board));
