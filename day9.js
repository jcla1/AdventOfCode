...
const noRows = R.length(board),
      noCols = R.length(board[0]);

const neighbours = (i, j) => {
  const candidates = [[i-1, j], [i+1, j], [i, j+1], [i, j-1]];
  return R.filter(([s, t]) => 0 <= s && s < noRows && 0 <= t && t < noCols, candidates);
}

const access = (board, [s,t]) => board[s][t],
      lowPoints = R.map(i => R.map(j => {
        const curVal = access(board, [i, j]),
              neighVals = R.map(pos => access(board, pos), neighbours(i, j));
...
const maximum = R.reduce(R.max, -Infinity),
      maxOfBoard = board => maximum(R.map(maximum, board)),
      boardSum = R.o(R.sum, R.map(R.sum));
...
function fillBasin(board, startPos) {
  // we want to stay functional!
  board = R.clone(board);

  var stack = [startPos], size = 0;
  while (R.length(stack) !== 0) {
    ...
    board[i][j] = 1; size++;
    stack = R.concat(stack, R.filter(([s, t]) => board[s][t] !== -1, neighbours(i, j)));
  }

  return [board, size];
}

function getBasinSizes(board) {
  var basinSizes = [];
  for (var i = 0; i < R.length(board); i++) {
    for (var j = 0; j < R.length(board[i]); j++) {
      if (board[i][j] === 0) {
...
console.log(R.compose(R.product, R.takeLast(3), R.sortBy(R.identity))(basinSizes));
