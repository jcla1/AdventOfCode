...
const countAllPaths = R.memoizeWith((init, goal) => `${init}|${goal}`,
  (initialState, goal, nextStates) => {
    if (R.equals(initialState, goal)) return 1;
    let total = 0;
    const neighbours = nextStates(initialState);
    for (let i = 0; i < neighbours.length; i++) {
      total += countAllPaths(neighbours[i], goal, nextStates);
    }
    return total;
});
...
module.exports = {
  ...
  toIntArr, reduce1,
  astar, findAllPaths, countAllPaths, foldM,
};
