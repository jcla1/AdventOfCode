...
// range.
const advanceRobot = R.curry(([sx, sy], [[x, y], [dx, dy]]) =>
  [[(x + dx + sx) % sx, (y + dy + sy) % sy], [dx, dy]]);
...
const getSafetyFactor = (states) => {
  const q1 = R.filter(([[x, y], _]) =>
    x <= boardSize[0]/2 - 1 && y <= boardSize[1]/2 - 1, states);
  const q2 = R.filter(([[x, y], _]) =>
    x > boardSize[0]/2 && y <= boardSize[1]/2 - 1, states);
  const q3 = R.filter(([[x, y], _]) =>
    x <= boardSize[0]/2 - 1 && y > boardSize[1]/2, states);
  const q4 = R.filter(([[x, y], _]) =>
    x > boardSize[0]/2 && y > boardSize[1]/2, states);
