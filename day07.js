...
const findOuterBagColors = R.curry((rules, color) => {
  ...
  return R.reduce(R.union, outerColors,
      R.map(findOuterBagColors(rules), outerColors));
});
...
const findNoBags = (rules, color) => R.reduce(
    (total, [count, innerColor]) =>
      total + count * findNoBags(rules, innerColor),
    1, rules[color]);
