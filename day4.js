...
const isValid = R.allPass(R.map(R.has, ['byr', 'iyr', 'eyr', 'hgt', 'hcl', 'ecl', 'pid']))
...
const isValidStrict = R.allPass([
  ...
  pp => {
    const [num, units] = R.slice(1, 3, R.match(/(\d+)(in|cm)/, pp['hgt'])),
          height       = parseInt(num);
    const lower = {'in': 59, 'cm': 150}, upper = {'in': 76, 'cm': 193};
    return lower[units] <= height && height <= upper[units];
  },
  R.o(R.test(/^#[0-9a-f]{6}$/), R.prop('hcl')),
  R.o(R.flip(R.includes)(['amb', 'blu', 'brn', 'gry', 'grn', 'hzl', 'oth']), R.prop('ecl')),
  R.o(R.test(/^\d{9}$/), R.prop('pid'))
]);
